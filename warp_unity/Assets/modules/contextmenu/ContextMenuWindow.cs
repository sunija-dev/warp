...
public class ContextMenuWindow : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private void Update()
    {
        // Maybe optional close if leave field, bcs clicks in gw2 are not collected -> implemented
        // alternatively: force that next click is received
    ...
    public void Init(List<ContextMenu.Option> _liOptions)
    {
        ...
        foreach (ContextMenu.Option option in _liOptions)
        {
            ...
            // text
            Languages.Set(goOption.GetComponentInChildren<TMP_Text>(), option.strNameKey);
